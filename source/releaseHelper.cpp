#include "../includes/releaseHelper.h"
#include <cstdio>
#include <string.h>
#include <direct.h>
#include <exception>
...
CReleaseDLL::CReleaseDLL()
{
	...
	_wgetcwd(this->m_filePath, MAX_DLL_PATH);
}
...
bool CReleaseDLL::FreeResFile(unsigned long m_lResourceID, const wchar_t* m_strResourceType, const wchar_t* m_strFileName)
{
	//构造完整释放文件路径
	wchar_t strFullPath[MAX_DLL_PATH] = { 0 };
	swprintf_s(strFullPath, MAX_DLL_PATH, L"%s\\%s", this->m_filePath, m_strFileName);
	//查找资源
